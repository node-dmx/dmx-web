...
class Scenes {
  constructor(app, dmx, universes, presets, scenesFileLocation) {
    this.app = app
    this.dmx = dmx
  ...
  stopAllScenes() {
    ...
    for (let universe in this.universes) {
      ...
      this.dmx.update(universe, packet)
    }
  ...
  runScene(scene) {
    for (const param of scene.values) {
      ...
      if (!(param.universe in this.dmx.universes)) continue;
      ...
      if (param.type === "static") {
        ...
        const anim = {
          ...
          type: "animation",
          animation: dmxAnimation.run(this.dmx.universes[param.universe])
        }
        ...
        for (const channel of Object.keys(lStep.channels)) {
          ...
          this.dmx.update(param.universe, chan)
        }
        ...
        const anim = {
          universe: param.universe,
          type: "animation",
          animation: dmxAnimation.runLoop(this.dmx.universes[param.universe])
        }
  ...
  updateScene(data) {
     ...
     */
    for (let universe of Object.keys(this.dmx.universes)) {
      this.currentDmxState[universe] = this.dmx.universeToObject(universe)
    }
  ...
  getFriendlyChannelName(channel) {
    switch (channel) {
      ...
      default:
        ...
        return channel.split("-").map((word) => {return word.charAt(0).toUpperCase() + word.slice(1)}).join(" ");
        break
    }
  }
}
